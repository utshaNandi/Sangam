...
const dashboardCode = `import { useState, useRef, useEffect } from 'react';
import { Icons } from './Icons';
import { tree, scheduleData, get20Students, getAssignmentsForSection } from './TeacherViews';
import teacher1Img from '../../components/teacher1.png';

export default function DashboardHome({ onNavigate }) {
  // Calculate Active Classes (Sections)
  let activeClassesCount = 0;
  const allSections = [];
  for (const b in tree) {
    for (const y in tree[b]) {
       for (const sec of tree[b][y]) {
         activeClassesCount++;
         allSections.push({ branch: b, year: y, section: sec });
       }
    }
  }

  // Calculate Total Students and Attendance
  const totalStudents = activeClassesCount * 20;
  let totalAttendance = 0;
  let needsAttention = 0;
  allSections.forEach(sec => {
    const sectionStudents = get20Students(sec.branch, sec.year, sec.section);
    sectionStudents.forEach(st => {
      // Deterministic attendance between 60 and 100
      const seedStr = st.id;
      let seed = 0;
      for(let i = 0; i < seedStr.length; i++) seed += seedStr.charCodeAt(i);
      const att = 60 + (seed % 41);
      totalAttendance += att;
      if (att < 75) needsAttention++;
    });
  });
  ...
  allSections.forEach(sec => {
    const sectionAssignments = getAssignmentsForSection(sec.branch, sec.year, sec.section);
  ...
  // Sort assignments to get recent ones
  const recentAssignments = allAssignments.slice(0, 3); // Get first 3 assignments to display

  // Process submitted counts for recent assignments
  recentAssignments.forEach(a => {
    // Extract branch, year, sec from className (format: "year • branch • section")
    const parts = a.className.split(' • ');
    if (parts.length === 3) {
      const yr = parts[0];
      const br = parts[1];
      const sc = parts[2];
      const studentsForAssignment = get20Students(br, yr, sc);
      let submitted = 0;
      studentsForAssignment.forEach(student => {
        const seedStr = a.id + student.id;
        let seed = 0;
        for(let i=0; i<seedStr.length; i++) seed += seedStr.charCodeAt(i);
        if (seed % 2 !== 0) submitted++;
      });
      a.submitted = submitted;
    } else {
      a.submitted = 0; // fallback
    }
  });

  // Calculate Today's Classes from Schedule
  const todayName = new Date().toLocaleDateString('en-US', { weekday: 'long' });
  const rawTodayClasses = scheduleData[todayName] || [];
  const todayClasses = rawTodayClasses.filter(c => c.type !== 'Break' && c.type !== 'Self Study');
    ...
    <div className="space-y-8 animate-fade-in relative">
      ...
      <div className="relative z-10 space-y-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-5">
          <StatCard title="My Classes" value={\`\${activeClassesCount} Active Classes\`} icon={<Icons.Classes />} onClick={() => onNavigate('My Classes')} />
          <StatCard title="Students" value={\`\${totalStudents} Students\`} icon={<Icons.Students />} onClick={() => onNavigate('Students')} />
          <StatCard title="Pending Assignments" value={\`\${pendingAssignmentsCount} To Review\`} icon={<Icons.Assignments />} onClick={() => onNavigate('Assignments')} />
          <StatCard title="Today's Classes" value={\`\${todayClasses.length} Scheduled\`} icon={<Icons.Schedule />} onClick={() => onNavigate('My Classes')} />
        </div>
              ...
              <div className="space-y-3 relative z-10">
                {todayClasses.length === 0 ? (
                  ...
                  todayClasses.map((c, idx) => (
                    <div key={idx} className="flex items-center justify-between p-4 rounded-2xl hover:bg-slate-50 transition-all duration-200 border border-transparent hover:border-slate-100 active:scale-[0.98] group cursor-pointer" onClick={() => onNavigate('My Classes')}>
                      <div className="flex items-center gap-5">
                        <div className="w-16 h-16 rounded-2xl bg-theme-bg text-theme-primary flex flex-col items-center justify-center flex-shrink-0 transition-transform duration-300 group-hover:scale-105">
                          <span className="text-[10px] font-bold uppercase tracking-wider">{c.time.split(' - ')[0].split(' ')[1] || 'AM'}</span>
                        ...
                        <div>
                          ...
                          <p className="text-sm text-slate-500 font-medium">{c.classInfo} • 20 Students</p>
                        </div>
                      </div>
          ...
          <div className="space-y-8">
            ...
            <section className="bg-white rounded-[24px] border border-slate-100 shadow-[0_2px_20px_rgba(0,0,0,0.02)] p-7">
              ...
              <div className="grid grid-cols-3 gap-3">
                ...
                <QuickAction icon={<Icons.Classes />} label="My Classes" onClick={() => onNavigate('My Classes')} />
              </div>
            </section>
            ...
            <section className="bg-white rounded-[24px] border border-slate-100 shadow-[0_2px_20px_rgba(0,0,0,0.02)] p-7">
              ...
              <div className="space-y-4">
                ...
                <div className="flex justify-between items-center pb-4 border-b border-slate-100">
                  ...
                  <span className="font-bold text-theme-primary text-lg">{avgAttendance}</span>
                </div>
                <div className="flex justify-between items-center">
                  <span className="text-slate-500 font-medium">Needs Attention</span>
                  <span className="font-bold text-rose-500 text-lg bg-rose-50 px-2 py-0.5 rounded-lg">{needsAttention}</span>
                </div>
              </div>
            </section>
      ...
      <div className="relative z-10 flex flex-col lg:flex-row justify-between items-start lg:items-center gap-16 w-full max-w-7xl mx-auto px-10 py-16 md:px-16 md:py-20 pointer-events-none">
        ...
        <div className="max-w-2xl flex-1 pointer-events-auto">
           ...
           <div className="flex items-center gap-4 mt-12">
             <button onClick={() => onNavigate('My Classes')} className="px-8 py-3.5 bg-theme-primary text-white rounded-xl font-semibold shadow-sm hover:opacity-90 hover:shadow-md hover:-translate-y-0.5 active:scale-95 transition-all duration-200 text-[15px]">
               View My Classes
             </button>
             <button onClick={() => onNavigate('Schedule')} className="px-8 py-3.5 bg-white border border-slate-200 text-slate-700 rounded-xl font-semibold shadow-sm hover:bg-slate-50 hover:border-slate-300 hover:shadow-md hover:-translate-y-0.5 active:scale-95 transition-all duration-200 text-[15px]">
               View Schedule
             </button>
           </div>
        </div>
...
function StatCard({ title, value, icon, onClick }) {
  return (
    <div onClick={onClick} className="bg-white p-6 rounded-[24px] border border-slate-100 shadow-[0_2px_20px_rgba(0,0,0,0.02)] flex items-start gap-4 transition-all duration-300 hover:-translate-y-1.5 hover:shadow-[0_8px_30px_rgba(0,0,0,0.06)] cursor-pointer group">
...
fs.writeFileSync('src/pages/Teacher/DashboardHome.jsx', dashboardCode);
console.log('Successfully updated DashboardHome.jsx');
