...
// The file currently has "export \n  if (selectedSection) {" around line 75.
// We need to replace everything from "export \n  if (selectedSection) {" upwards to "};" of scheduleData.
const startReplace = content.indexOf('export \n  if (selectedSection) {');
if (startReplace !== -1) {
    const head = content.substring(0, startReplace);
    const tail = content.substring(startReplace + 'export \n'.length);
    content = head + classesViewCode + tail;
    fs.writeFileSync('src/pages/Teacher/TeacherViews.jsx', content);
    console.log('Fixed ClassesView and StudentsView header');
} else {
    console.log('Could not find error marker');
}
