          ...
          <div className="lg:col-span-2 space-y-8">
            ...
            <section className="bg-white rounded-[24px] border border-slate-100 shadow-[0_2px_20px_rgba(0,0,0,0.02)] p-7 relative overflow-hidden">
              ...
              <div className="space-y-3 relative z-10">
                {todayClasses.map(c => (
                  <div key={c.id} className="flex items-center justify-between p-4 rounded-2xl hover:bg-slate-50 transition-all duration-200 border border-transparent hover:border-slate-100 group cursor-pointer" onClick={() => onNavigate('My Classes')}>
                    <div className="flex items-center gap-5">
                      <div className="w-16 h-16 rounded-2xl bg-theme-bg text-theme-primary flex flex-col items-center justify-center flex-shrink-0">
            ...
            <section className="bg-white rounded-[24px] border border-slate-100 shadow-[0_2px_20px_rgba(0,0,0,0.02)] p-7">
              <div className="flex justify-between items-center mb-5">
                ...
                <button onClick={() => onNavigate('Assignments')} className="text-sm text-theme-primary hover:opacity-80 font-bold transition-colors">View All →</button>
              </div>
              <div className="space-y-3">
                {assignments.map(a => (
                  <div key={a.id} className="flex flex-col sm:flex-row sm:items-center justify-between p-4 rounded-2xl border border-slate-100 hover:border-theme-primary/30 hover:shadow-sm transition-all duration-200 cursor-pointer" onClick={() => onNavigate('Assignments')}>
                    <div>
                    ...
                    <div className="mt-3 sm:mt-0 flex items-center gap-5">
                      ...
                      <div className="w-20 h-2.5 bg-slate-100 rounded-full overflow-hidden">
                        <div className="h-full bg-theme-primary rounded-full" style={{ width: `${(a.submitted / a.total) * 100}%` }}></div>
                      </div>
                    </div>
                  </div>
                ))}
      ...
      <div className="relative z-10 flex flex-col md:flex-row justify-between items-start md:items-center gap-8">
        <div className="max-w-2xl">
           ...
           <div className="flex items-center gap-4 mt-8">
             <button className="px-6 py-3 bg-theme-primary text-white rounded-xl font-bold shadow-sm hover:opacity-90 hover:shadow-md hover:-translate-y-0.5 transition-all">
               Start Today's Session
             </button>
             <button className="px-6 py-3 bg-theme-bg text-theme-primary rounded-xl font-bold shadow-sm hover:bg-slate-100 hover:-translate-y-0.5 transition-all">
               View Schedule
             </button>
           </div>
        </div>
...
function StatCard({ title, value, icon }) {
  return (
    <div className="bg-white p-6 rounded-[24px] border border-slate-100 shadow-[0_2px_20px_rgba(0,0,0,0.02)] flex items-start gap-4 transition-transform hover:-translate-y-1 duration-300">
      <div className="w-12 h-12 rounded-2xl bg-theme-bg text-theme-primary flex items-center justify-center flex-shrink-0">
        {icon}
      </div>
      <div>
...
function QuickAction({ icon, label, onClick }) {
  return (
    <button 
      onClick={onClick}
      className="flex flex-col items-center justify-center gap-3 p-4 rounded-2xl border border-slate-100 hover:border-theme-primary/30 hover:bg-theme-bg transition-all text-slate-500 hover:text-theme-primary group"
    >
