...
export function ClassesView() {
  ...
  const todaysSchedule = scheduleData[currentDay] || [];
  
  // Mock current student profile as per instructions
  const studentProfile = { year: '2nd Year', branch: 'CSE', section: 'Section A' };
  const expectedClassInfo = \`\${studentProfile.year} • \${studentProfile.branch} • \${studentProfile.section}\`;

  const myClasses = todaysSchedule
    .filter(period => period.classInfo && period.classInfo.includes(expectedClassInfo))
    .map(period => ({
      id: period.period,
